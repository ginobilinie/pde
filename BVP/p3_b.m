...
plot(x,u);
saveas(gcf,'estimatedU-bvpoder4.png');
%%%%%%% Plot error
figure(2); plot(x,U-u)
saveas(gcf,'plotoferror-bvpoder4.png');
norm(U-u,inf) %%% Print out the maximum error
return

function [x,U] = BVP_order4(a,b,ua,ub,f,n)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% This matlab function BVP_oder2 solves the following two-point 
% boundary value problem: u''(x) = f(x) using the 4th order scheme,
%and suppose it can give a 2nd order convergence 
% Input: 
% a, b: Two end points. 
...
for i=2:n-2,
    x(i) = a+i*h;
    F(i) = f(x(i))+1/12*(f(x(i-1))-2*f(x(i))+f(x(i+1)));
end
F(1)=f(x(1))+1/12*(f(a)-2*f(x(1))+f(x(2)));
F(1) = F(1) - ua/h2;
F(n-1)=f(x(n-1))+1/12*(f(x(n-2))-2*f(x(n-1))+f(b));
F(n-1) = F(n-1) - ub/h2;
U = A\F;
return
%%%%%--------- End of the program -------------------------------------
